      ...
      {projects.map((p, index) => (
        <div key={index} className='flex items-start justify-center xs:w-full xs:h-full lg:w-[30vw] p-4 rounded-lg bg-transparent'>
          ...
          <div className="flex flex-col w-full h-full">
            ...
            <h2 className="text-gray-400 font-sans xs:max-sm:text-sm ">{p.empresa}</h2>
            <h3 className="text-white font-sans xs:max-sm:text-sm ">{p.time}</h3>
            <h3 className="text-white font-sans xs:max-sm:text-sm ">{p.local}</h3>

            <p className="text-white font-sans mt-5 xs:max-sm:text-sm ">{p.description}</p>
          </div>
        </div>
      ))}
    </>
